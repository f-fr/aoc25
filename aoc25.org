...
#+END_SRC
Now we compute for each square the number of ~@~ neighbors. For empty
squares we set the number of neighbors to something large. This allows
us to recognize (and ignore) empty squares later by only looking at
that square's degree.
#+BEGIN_SRC zig -i :exports code :tangle src/04/run.zig
...
#+END_SRC
Squares with degree < 4 can be removed. We collect all of them in a
list.
#+BEGIN_SRC zig -i :exports code :tangle src/04/run.zig
